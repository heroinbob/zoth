defmodule Zoth.Token.Strategy.PasswordTest do
  use Zoth.TestCase

  alias Zoth.{Config, Token, Token.Password}
  ...
  setup do
    user = Fixtures.insert(:user, email: @username)
  ...
  test "#grant/2 error when missing required values" do
    Enum.each(["username", "password"], fn k ->
      ...
      assert Token.grant(params, otp_app: :zoth) ==
  ...
  test "#grant/1 error when no password auth set" do
    ...
    assert Password.grant(@valid_request, otp_app: :zoth, password_auth: nil) ==
  ...
  test "#grant/1 returns access token when only client_id required", %{
    ...
    assert {:ok, body} = Token.grant(params, otp_app: :zoth)
    access_token = QueryHelpers.get_latest_inserted(OauthAccessToken)
  ...
  test "#grant/1 doesn't set refresh_token when Zoth.Config.use_refresh_token? == false" do
    assert {:ok, body} =
             Password.grant(@valid_request,
               otp_app: :zoth,
  ...
  test "#grant/1 returns access token with limited scope" do
    ...
    assert {:ok, _} = Token.grant(params, otp_app: :zoth)
    access_token = QueryHelpers.get_latest_inserted(OauthAccessToken)
