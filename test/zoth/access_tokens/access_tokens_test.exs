defmodule Zoth.AccessTokensTest do
  use Zoth.TestCase
  ...
  test "get_by_token/2", %{user: user} do
    assert {:ok, access_token} =
             AccessTokens.create_token(user, %{}, otp_app: :zoth)

    assert %OauthAccessToken{id: id} =
             AccessTokens.get_by_token(access_token.token, otp_app: :zoth)
  ...
  describe "get_by_previous_refresh_token_for/3" do
    ...
    test "with application", %{user: user, application: application} do
      ...
      new_application = Fixtures.insert(:application, owner: user, uid: "new_app")
  ...
  describe "get_token_for/4" do
    test "fetches for resource owner", %{user: user, application: application} do
      {:ok, access_token1} = AccessTokens.create_token(user, %{}, otp_app: :zoth)
      ...
      {:ok, access_token2} = AccessTokens.create_token(user, %{}, otp_app: :zoth)

      {:ok, _access_token_with_application} =
        AccessTokens.create_token(user, %{application: application}, otp_app: :zoth)

      assert %OauthAccessToken{id: id} =
               AccessTokens.get_token_for(user, nil, nil, otp_app: :zoth)
    ...
    test "with application", %{user: user, application: application} do
      ...
      assert %OauthAccessToken{id: id} =
               AccessTokens.get_token_for(user, application, nil, otp_app: :zoth)
    ...
    test "with scopes", %{user: user} do
      {:ok, access_token1} =
        AccessTokens.create_token(user, %{scopes: "public"}, otp_app: :zoth)

      {:ok, access_token2} =
        AccessTokens.create_token(user, %{scopes: "read write"}, otp_app: :zoth)

      assert %OauthAccessToken{id: id} =
               AccessTokens.get_token_for(user, nil, "public", otp_app: :zoth)

      assert id == access_token1.id

      assert %OauthAccessToken{id: id} =
               AccessTokens.get_token_for(user, nil, "write read", otp_app: :zoth)

      assert id == access_token2.id

      refute AccessTokens.get_token_for(user, nil, "other_read", otp_app: :zoth)
    end

    test "filters revoked", %{user: user} do
      {:ok, access_token} = AccessTokens.create_token(user, %{}, otp_app: :zoth)
      assert AccessTokens.get_token_for(user, nil, nil, otp_app: :zoth)

      AccessTokens.revoke(access_token)
      refute AccessTokens.get_token_for(user, nil, nil, otp_app: :zoth)
    end
  ...
  test "get_authorized_tokens_for/2", %{user: user, application: application} do
    ...
    assert [%OauthAccessToken{id: id}] =
             AccessTokens.get_authorized_tokens_for(user, otp_app: :zoth)
    ...
    AccessTokens.revoke(access_token, otp_app: :zoth)
    assert AccessTokens.get_authorized_tokens_for(user, otp_app: :zoth) == []

    assert AccessTokens.get_authorized_tokens_for(Fixtures.insert(:user),
             otp_app: :zoth
           ) == []
  end
  ...
  describe "create_token/3" do
    ...
    test "doesn't add refresh token when disabled", %{user: user} do
      {:ok, access_token} =
        AccessTokens.create_token(user, %{use_refresh_token: false}, otp_app: :zoth)
    ...
    test "with custom scopes", %{user: user} do
      assert {:ok, access_token} =
               AccessTokens.create_token(user, %{scopes: "read"}, otp_app: :zoth)
    ...
    test "with invalid scopes", %{user: user} do
      assert {:error, changeset} =
               AccessTokens.create_token(user, %{scopes: "invalid"}, otp_app: :zoth)
  ...
  describe "revoke/2" do
    test "revokes token", %{user: user} do
      ...
      assert {:ok, access_token} = AccessTokens.revoke(access_token, otp_app: :zoth)
    ...
    test "doesn't revoke revoked tokens", %{user: user} do
      ...
      {:ok, access_token2} = AccessTokens.revoke(access_token, otp_app: :zoth)
