defmodule Mix.Tasks.Zoth.Gen.SchemasTest do
  use Zoth.Mix.TestCase

  alias Mix.Tasks.Zoth.Gen.Schemas
  ...
  test "it uses the configured otp_app when --context-app is not given" do
    File.cd!(@tmp_path, fn ->
      # Test config defines :zoth as the otp_app
      root_path = Path.join(["lib", "zoth"])
      ...
      for file <- @required_files do
        ...
        assert File.read!(path) =~ "use #{inspect(macro)}, otp_app: :zoth"
      end
    end)
  end
