defmodule Zoth.Config do
  ...
  def repo(config) do
    get(config, :repo) ||
      raise """
      No `:repo` found in Zoth configuration.

      Please set up the repo in your configuration:

      config #{inspect(Keyword.get(config, :otp_app, :zoth))}, Zoth,
        repo: MyApp.Repo
      """
  end
  ...
  The behavior Zoth.Behaviors.SkipAuthorization is provided to help
  ...
  def skip_authorization(config) do
    get(
      ...
      &Zoth.Features.skip_authorization?/2
    )
  end
